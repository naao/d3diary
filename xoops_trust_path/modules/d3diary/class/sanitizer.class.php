<?php
if( ! class_exists( 'D3diarySanitizer' ) ) {

class D3diarySanitizer {

/*
 * included by naao Aug/27/2016
 *
 ...
 */
	...
	*/	 
	...
	private function RLfiDt($valueInc, $func, $baseDir){	
	...
	*/		
	private function OSCmd($valueInc){
	...
	*/	
	private function EvalFunc($valueInc, $lineVul=NULL){
		// newline check
		$nl_1 = chr(10);
		$nl_2 = chr(13);
		...
		if($lineVul != NULL){
			$out = str_replace($valueInc, $out, $lineVul);
		}
		return $out;	
	}
...
//--------------------------
	...
	* @description: Filter the content by method and verify if exists RFI, LFI, DT, SCD
	*/	
	public function san_mix($valueInc, $func, $baseDir){
		return $this->RLfiDt($valueInc, $func, $baseDir);
	}
	...
	*/
	public function san_osci($valueInc){
		return $this->OScmd($valueInc);
	}	
	...
	*/	
	public function san_eval($valueInc, $lineVul=NULL){
		return $this->EvalFunc($valueInc, $lineVul);
	}
