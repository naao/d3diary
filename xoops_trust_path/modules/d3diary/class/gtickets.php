...
class XoopsGTicket {
	...
	function draw_repost_form( $area = '' )
	{
		...
		foreach( $_POST as $key => $val ) {
			...
			if( is_array( $val ) ) {
				...
				$form .= '<input type="hidden" name="'.htmlspecialchars($key,ENT_QUOTES).'" value="'.htmlspecialchars($val,ENT_QUOTES,_CHARSET).'" />'."\n" ;
			}
		}
		$table .= '</table>' ;
	...
	function errorHandler4FindOutput($errNo, $errStr, $errFile, $errLine)
	{
		if( preg_match( '?'.preg_quote(XOOPS_ROOT_PATH).'([^:]+)\:(\d+)?' , $errStr , $regs ) ) {
			echo "Irregular output! check the file ".htmlspecialchars($regs[1],ENT_QUOTES,_CHARSET)." line ".htmlspecialchars($regs[2],ENT_QUOTES,_CHARSET) ;
		} else {
